...
export async function getGinnyGif(): Promise<string> {
    const tenorApiKey = env.TENOR_API_KEY;

    const res = await fetch(`https://api.tenor.com/v1/search?q=ginny-weasley&limit=25&key=${tenorApiKey}`);
    ...
    const gifs = json.results
        .filter(result => result.content_description === 'Ginny Weasley GIF')
